...
def plot_comparison(all_results, save_dir: Path):
   ...
   fig.suptitle('Model Comparison: GRU vs CNN vs GRU_CNN', fontsize=16, fontweight='bold')
   ...
   for result in all_results:
       ...
       ax.plot(history['val_loss'], label=f"{model_type} Val", linestyle='--', alpha=0.7)
   ax.set_xlabel('Epoch')
...
def generate_report(all_results, save_dir: Path):
   ...
   report.append("| Model | Parameters | Best Val Loss | Test MAE | Test RMSE | Training Epochs |\n")
   report.append("|-------|-----------|---------------|----------|-----------|----------------|\n")
   ...
   for result in all_results:
       ...
       report.append(f"| {model_type} | {params} | {best_val} | {test_mae} | {test_rmse} | {epochs} |\n")
   ...
   for result in all_results:
       ...
       if 'distance_errors' in result['results']:
           ...
           if isinstance(e_24h, float):
               report.append(f"| {model_type} | {e_24h:.2f} | {e_48h:.2f} | {e_72h:.2f} |\n")
           else:
               report.append(f"| {model_type} | {e_24h} | {e_48h} | {e_72h} |\n")
   ...
   report.append(f"- Lowest validation loss: {best_model['best_val_loss']:.4f}\n")
   ...
   for result in all_results:
       model_type = result['model_type']
       if model_type == 'GRU':
           report.append(f"- **{model_type}**: Good for temporal patterns, lowest parameters\n")
       elif model_type == 'CNN':
           report.append(f"- **{model_type}**: Good for local patterns, fast inference\n")
       elif model_type == 'GRU_CNN':
           report.append(f"- **{model_type}**: Hybrid approach, captures both spatial and temporal\n")
...
def main():
   ...
   for model_type in ['GRU', 'CNN', 'GRU_CNN']:
       try:
           result = train_and_evaluate_model(model_type, config, data, save_dir)
