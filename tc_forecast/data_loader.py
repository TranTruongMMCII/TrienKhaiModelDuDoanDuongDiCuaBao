...
class TCDataLoader:
    ...
    def compute_derived_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['P'] = df[pres_col].astype(float) if pres_col in df.columns else 1013.0
        ...
        for storm_id in df[sid_col].unique():
            ...
            for i in range(1, n):
                ...
                dis, ang = get_distance(lat1, lon1, lat2, lon2, self.EARTH_RADIUS)
                ...
                # X = R * cos(lat) * dlon
                # Y = R * dlat
                x_dist = self.EARTH_RADIUS * np.cos(np.radians(lat2)) * np.radians(dif_lon)
        ...
        print(f"Computed 19 derived features for {len(df[sid_col].unique())} storms")
    ...
    def load_environmental_data(self) -> Dict[str, np.ndarray]:
        ...
        atmos_dir = self.data_config.get('atmospheric_data_dir', '../atmospheric_data')
        atmos_dir = resolve_path(atmos_dir, Path(__file__).parent)
        
        if not Path(atmos_dir).exists():
            print(f"Warning: Atmospheric data directory not found: {atmos_dir}")
            return env_data
        ...
        uv_files = ['u300', 'u500', 'u700', 'u850', 'v300', 'v500', 'v700', 'v850']
        uv_data = {}
        for uv_file in uv_files:
            uv_path = Path(atmos_dir) / f'{uv_file}.csv'
            if uv_path.exists():
                print(f"Loading {uv_file} data...")
                uv_data[uv_file] = load_atmospheric_csv_by_coords(str(uv_path), 'VALUE')
        ...
        for z_file in z_files:
            ...
            if z_path.exists():
                ...
                z_data[z_file] = load_atmospheric_csv_by_coords(str(z_path), 'VALUE')
    ...
    def create_track_sequences(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, List[Dict]]:
        ...
        for storm_id, storm_df in grouped:
            ...
            missing_cols = [col for col in feature_cols if col not in storm_df.columns]
            if missing_cols:
                print(f"Warning: Storm {storm_id} missing columns: {missing_cols}")
                continue
    ...
    def create_environmental_sequences(self, env_data: Dict, 
        ...
        if 'sst_df' in env_data:
            ...
            if sst_list:
                ...
                match_pct = matched_sst / len(sst_list) * 100 if sst_list else 0
                print(f"SST sequences shape: {env_sequences['sst'].shape}, matched: {match_pct:.1f}%")
        
        # ===== Process UV data (EXACT MATCH by STORM_ID+LAT+LONG+TIME) =====
        if 'uv_dict' in env_data:
            uv_dict = env_data['uv_dict']
            uv_vars = ['u300', 'u500', 'u700', 'u850', 'v300', 'v500', 'v700', 'v850']
                ...
                if target_idx < len(storm_df):
                    ...
                    for var in uv_vars:
                        if var in uv_dict and key in uv_dict[var]:
                            ...
                            channels.append(np.zeros((21, 21), dtype=np.float32))
                else:
                    channels = [np.zeros((21, 21), dtype=np.float32) for _ in uv_vars]
            ...
            if uv_list:
                ...
                print(f"UV sequences shape: {env_sequences['uv'].shape}, matched: {match_pct:.1f}%")
        ...
        if 'z_dict' in env_data:
                ...
                if target_idx < len(storm_df):
                    ...
                    for var in z_vars:
                        if var in z_dict and key in z_dict[var]:
                            ...
                            channels.append(np.zeros((z_height, z_width), dtype=np.float32))
                else:
                    channels = [np.zeros((z_height, z_width), dtype=np.float32) for _ in z_vars]
            ...
            if z_list:
                ...
                print(f"Z sequences shape: {env_sequences['z'].shape}, matched: {match_pct:.1f}%")
    ...
    def normalize_data(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        X_train_norm = self.scaler_track.transform(X_train_2d).reshape(n_train, seq_len, n_features)
        ...
        if X_val is not None:
            ...
            X_val_norm = self.scaler_track.transform(X_val_2d).reshape(n_val, seq_len, n_features)
        ...
        if X_test is not None:
            ...
            X_test_norm = self.scaler_track.transform(X_test_2d).reshape(n_test, seq_len, n_features)
        ...
        env_results = self._normalize_environmental_data(env_train, env_val, env_test)
    ...
    def _normalize_environmental_data(self, env_train: Dict, env_val: Dict, env_test: Dict
        ...
        for key in env_train.keys():
            ...
            env_train_norm[key] = scaler.transform(data_2d).reshape(original_shape)
            ...
            if env_test and key in env_test:
                ...
                env_test_norm[key] = scaler.transform(test_2d).reshape(test_shape)
    ...
    def load_and_prepare(self, track_path: str = None, load_env: bool = True) -> Dict:
        ...
        if load_env:
            ...
            if raw_env_data:
                env_data = self.create_environmental_sequences(raw_env_data, df, metadata)
...
# ============================================================================
    ...
    def load_multiple_csvs(self, atmospheric_data_dir: str = None) -> pd.DataFrame:
        ...
        """
        warnings.warn("load_multiple_csvs is deprecated. Use load_and_prepare() instead.")
    ...
    def create_sequences(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, List[Dict]]:
        ...
        """
        warnings.warn("create_sequences is deprecated. Use create_track_sequences() instead.")
