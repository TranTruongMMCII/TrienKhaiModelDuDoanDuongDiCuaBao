...
class TCTrainer:
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray,
              ...
              env_train: Dict[str, np.ndarray] = None,
              env_val: Dict[str, np.ndarray] = None
              ) -> keras.callbacks.History:
        """
        Train the model

        Args:
            X_train: Training track sequences (n_samples, seq_len, n_features)
            y_train: Training targets (n_samples, 24)
            X_val: Validation track sequences (optional)
            y_val: Validation targets (optional)
            env_train: Environmental training data dict with keys 'uv', 'sst', 'z' (optional)
            env_val: Environmental validation data dict (optional)
        ...
        # Prepare training input based on model type
        model_type = self.config.model.type.upper()

        if model_type == "GRU_CNN" and env_train is not None:
            # Multi-input for GRU-CNN: [track, uv, sst, z]
            train_inputs = self._prepare_multi_input(X_train, env_train)
            val_inputs = None
            if X_val is not None and y_val is not None and env_val is not None:
                val_inputs = self._prepare_multi_input(X_val, env_val)
            ...
            # Single input for GRU-only model
            train_X = X_train
            n_train = len(X_train)
        ...
        if env_train is not None:
            print(f"  Environmental data: {list(env_train.keys())}")
        print()
    ...
    def _prepare_multi_input(self, X_track: np.ndarray,
                             env_data: Dict[str, np.ndarray]) -> List[np.ndarray]:
        """
        Prepare multi-input data for GRU-CNN model.

        Args:
            X_track: Track sequences (n_samples, seq_len, n_features)
            env_data: Dict with 'uv', 'sst', 'z' arrays

        Returns:
            List of [track, uv, sst, z] arrays for Keras multi-input
        """
        n_samples = len(X_track)

        # Get UV data or create zeros
        if 'uv' in env_data and env_data['uv'] is not None:
            uv = env_data['uv']
        else:
            # Default shape: (n_samples, 8, 21, 21)
            uv = np.zeros((n_samples, 8, 21, 21), dtype=np.float32)

        # Get SST data or create zeros
        if 'sst' in env_data and env_data['sst'] is not None:
            sst = env_data['sst']
        else:
            # Default shape: (n_samples, 1, 21, 21)
            sst = np.zeros((n_samples, 1, 21, 21), dtype=np.float32)

        # Get Z data or create zeros
        if 'z' in env_data and env_data['z'] is not None:
            z = env_data['z']
        else:
            # Default shape: (n_samples, 3, 46, 81)
            z = np.zeros((n_samples, 3, 46, 81), dtype=np.float32)

        return [X_track, uv, sst, z]
    ...
    def _plot_training_history(self):
        ...
        if 'mean_absolute_error' in history:
            ...
            if 'val_mean_absolute_error' in history:
                axes[1].plot(history['val_mean_absolute_error'],
                             label='Val MAE')
    ...
    def evaluate(self, X_test: np.ndarray, y_test: np.ndarray,
                 ...
                 env_test: Dict[str, np.ndarray] = None
                 ) -> Dict[str, any]:
        """
        Evaluate model on test set

        Args:
            X_test: Test track sequences
            ...
            env_test: Environmental test data dict with keys 'uv', 'sst', 'z' (optional)
        ...
        if model_type == "GRU_CNN" and env_test is not None:
            test_inputs = self._prepare_multi_input(X_test, env_test)
        else:
            test_inputs = X_test

        # Make predictions
        y_pred = self.model.predict(test_inputs, batch_size=32)

        # Inverse transform if data_loader provided
        if data_loader is not None:
            y_pred_denorm, y_test_denorm = data_loader.inverse_transform_predictions(
                y_pred, y_test)
        else:
        ...
        if key_hours:
            ...
            for target_hour in key_hours:
                ...
                if 0 <= step_idx < len(mean_errors):
                    print(
                        f"{target_hour:>10} {mean_errors[step_idx]:>15.2f} {std_errors[step_idx]:>12.2f}")
            print(f"{'='*40}")
        ...
        if key_hours:
            ...
            for target_hour in key_hours:
                ...
                if 0 <= step_idx < len(rmse_per_step):
                    print(
                        f"{target_hour:>10} {rmse_per_step[step_idx]:>12.4f} {mae_per_step[step_idx]:>12.4f}")
            print(f"{'='*36}")
